...
In particular the `dfm_group` function is handy as it allows to concatenate documents based on document variables such as codings, party ids or election dates. Quanteda allows to calculate term-frequency inverse document frequency scores with the function `dfm_tfidf`. Tfidf scores are often used to identify words that are frequent and distinct for a given set of documents. We replicate here the example that we also used in the tidytext tutorial and calculate tfidf scores for different code categories for the Irish manifestos of 2016. The `topfeatures()` function returns the features with the highest tfidf-score.  
...
quanteda_irish %>% 
  ...
  mutate(
    cmp_code = str_sub(row, start = 1, end = 3),
    term = str_sub(row, start = 5, end = -1)
  ) %>%
  mutate(cmp_code = factor(cmp_code, labels = c("Decentralisation","Technology & Infrastructure","Environmental Protection","Culture"))) %>%
  group_by(cmp_code) %>%
  top_n(10, value) %>% 
  ggplot(aes(x = reorder(term, value) , y = value, fill = cmp_code)) +
...
The scores derived here differ in magnitude from the scores shown in the tidytext tutorial as the [tfidf calculation is different](https://stackoverflow.com/questions/48806699/tidytext-quanteda-and-tm-returning-different-tf-idf-scores).
