...
quanteda_irish %>% 
  ...
  gather(word, tfidfscore, -document) %>%
  rename(cmp_code = document) %>%
  group_by(cmp_code) %>%
  top_n(10, tfidfscore) %>% 
  ggplot(aes(x = reorder(word, tfidfscore) , y = tfidfscore, fill = cmp_code)) +
