...
mpds <- mp_maindataset(version ="2017b")
...
This introduction should give a brief overview of the Manifesto Project methodology as well as illustrate the structure of the Manifesto Project Main Dataset and the Manifesto Corpus. 
...
- Countries: Democratic countries, mostly member countries of the OECD as well as many Central and Eastern Europe countries.[^1] 

[^1]: _Exceptions_: In the past, a few countries and elections have been sampled that were not (fully-)democratic or free and fair, e.g. the coding of elections in Azerbaijan or Belarus. 

- Elections: Parliamentary (lower house) elections since the first democratic election in a country (and earliest since the end of 2nd world war).[^2]

[^2]: _Exceptions_: In South American countries we mostly collect and code programmes issued at presidential elections. 

- Parties: Programs of parties that gained at least one seat in parliament at the focal election.[^3]

[^3]: _Exceptions_: There are some exceptions from this rule. 
...
- Documents: An authoritative document enacted and published by a party before an election that outlines a party's policy plan for the time after the election and covers a broad range of policy issues.[^4]

[^4]: _Exceptions_: Some parties do not publish electoral programmes. 
...
The coding follows strict rules that are described in detail in the coding instructions. Despite the long history of the project, the general coding methodology has only slightly changed over time which makes the data comparable over time. The [current version of the coding instructions](https://manifesto-project.wzb.eu/information/documents/handbooks) can be found on the website.[^5]

[^5]: The coding instructions have slightly changed over time. [All major changes are well documented](https://manifesto-project.wzb.eu/down/papers/Evolution_of_the_Manifesto_Coding_Instructions_and_the_Category_Scheme.pdf).
...
not yet done.
```{r coverage, eval=FALSE, fig.cap=, include=FALSE}
